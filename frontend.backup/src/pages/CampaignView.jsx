...
// Tab: Personajes
// ============================================================================
export function CharactersTab({ campaignId, isGM }) {
  const navigate = useNavigate()
  ...
  const handleCharacterUpdate = async (updatedCharacter) => {
    // Actualizar personaje en la lista
    setCharacters(prev =>
  ...
  return (
    <div className="max-w-4xl mx-auto py-4">
      <div className="mb-6 flex items-center justify-between">
        <h2 className="text-2xl font-bold text-yellow-600">Personajes</h2>
        <button
          onClick={() => navigate(`/campaign/${campaignId}/create-character`)}
          className="flex items-center gap-2 px-6 py-2 bg-yellow-600 hover:bg-yellow-700 text-gray-900 font-bold rounded-lg transition-colors"
        >
          <Plus size={20} />
          Crear Personaje
        </button>
      ...
      {characters.length === 0 ? (
        <div className="text-center py-12 bg-gray-800/40 rounded-2xl border border-gray-700/50">
          <p className="text-gray-400 mb-4">Aún no hay personajes en esta campaña.</p>
          <button
            onClick={() => navigate(`/campaign/${campaignId}/create-character`)}
            className="inline-flex items-center gap-2 px-6 py-2 bg-yellow-600 hover:bg-yellow-700 text-gray-900 font-bold rounded-lg transition-colors"
          >
            <Plus size={20} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {characters.map((character) => (
            <div
              key={character.id}
              className="bg-gray-800 border border-gray-700 rounded-lg p-6 hover:border-yellow-600/50 transition-colors cursor-pointer"
              onClick={() => setSelectedCharacter(character)}
            >
              <h3 className="text-xl font-bold text-yellow-600 mb-2">{character.name}</h3>
              <div className="space-y-1 text-sm text-gray-400">
                <p><span className="text-gray-500">Raza:</span> {character.race}</p>
                <p><span className="text-gray-500">Clase:</span> {character.class_}</p>
                <p><span className="text-gray-500">Nivel:</span> {character.level}</p>
                <p><span className="text-gray-500">HP:</span> {character.hp_current}/{character.hp_max}</p>
              </div>
            </div>
          ))}
        </div>
...
// ============================================================================
// Tab: Mi Personaje (Para Jugadores)
// ============================================================================
export function MyCharacterTab({ campaignId, userEmail }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        // Obtener personaje del usuario en esta campaña
        const res = await characterAPI.listByCampaign(campaignId)
        const characters = res.data || []
        // Filtrar para usuario actual - en la BD el campo es player_id 
        // pero los personajes devueltos desde listByCampaign deberían ser del usuario actual
        const playerChar = characters.length > 0 ? characters[0] : null
    ...
    return (
      <div className="max-w-2xl mx-auto py-12 text-center">
        <div className="bg-amber-900/20 border border-amber-500/30 rounded-xl p-8">
          ...
          <h3 className="text-white font-bold text-lg mb-2">Sin Personaje</h3>
          <p className="text-gray-400 text-sm">Aún no tienes un personaje asignado a esta campaña.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto py-4">
      ...
      <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 border border-yellow-500/30 rounded-2xl p-8 mb-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-3xl font-bold text-yellow-400 mb-1">{character.name}</h2>
            <p className="text-sm text-gray-400">{character.race} {character.class} • Nivel {character.level}</p>
          </div>
          <button
            onClick={() => setShowLevelUpModal(true)}
            className="px-6 py-3 bg-yellow-600 hover:bg-yellow-500 text-gray-900 font-bold rounded-lg transition-colors flex items-center gap-2"
          >
            <Edit2 size={18} />
            Editar Estadísticas
          </button>
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-3 mb-6">
          {[
            ...
            const statsObj = typeof character.stats === 'string' ? JSON.parse(character.stats) : (character.stats || {})
            const value = statsObj[stat.key] || 10
            const parsed = parseInt(value)
            const modifier = Math.floor((parsed - 10) / 2)
            return (
              <div key={stat.label} className="bg-black/40 border border-yellow-500/30 rounded-lg p-3 text-center">
                <p className="text-xs text-gray-400 uppercase tracking-wider">{stat.label}</p>
                <p className="text-xl font-bold text-yellow-400">{parsed}</p>
                <p className="text-xs text-gray-500">{modifier > 0 ? '+' : ''}{modifier}</p>
              </div>
            )
          })}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <div className="bg-black/30 rounded-lg p-3">
            ...
            <p className="text-xl font-bold text-red-400">{character.hp_current || 0} / {character.hp_max || 0}</p>
          </div>
          <div className="bg-black/30 rounded-lg p-3">
            <p className="text-xs text-gray-500 uppercase tracking-wider">AC</p>
            <p className="text-xl font-bold text-blue-400">{character.armor_class || 10}</p>
          </div>
          <div className="bg-black/30 rounded-lg p-3">
            <p className="text-xs text-gray-500 uppercase tracking-wider">Velocidad</p>
            <p className="text-xl font-bold text-green-400">{character.speed || 30}ft</p>
          </div>
          <div className="bg-black/30 rounded-lg p-3">
            <p className="text-xs text-gray-500 uppercase tracking-wider">Alineación</p>
            <p className="text-xl font-bold text-purple-400">{character.alignment || 'N/A'}</p>
          </div>
        </div>
      </div>
...
// ============================================================================
// Tab: Inventario (Para Jugadores)
// ============================================================================
export function InventoryTab({ campaignId, userEmail }) {
  ...
  const loadInventory = async () => {
    try {
      ...
      if (playerChar && playerChar.equipment) {
        // equipment es JSON string o array
        const inv = typeof playerChar.equipment === 'string'
          ? JSON.parse(playerChar.equipment)
          : (Array.isArray(playerChar.equipment) ? playerChar.equipment : [])
        setInventory(inv)
      } else {
        setInventory([])
      }
  ...
  const handleAddItem = async () => {
    ...
    setAddingItem(true)
    try {
      const newInventory = [...inventory, { id: Date.now(), name: newItem, quantity: 1 }]
  ...
  return (
    <div className="max-w-3xl mx-auto py-4">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-yellow-400">Inventario</h2>
        <button
          onClick={() => setShowAddForm(!showAddForm)}
          className="flex items-center gap-2 px-4 py-2 bg-yellow-600 hover:bg-yellow-500 text-gray-900 font-bold rounded-lg transition-colors"
        >
          <Plus size={18} />
          Añadir Item
        </button>
      ...
      {showAddForm && (
        <div className="bg-gray-800/50 border border-yellow-500/30 rounded-xl p-4 mb-6 flex gap-3">
          <input
            type="text"
            value={newItem}
            onChange={e => setNewItem(e.target.value)}
            onKeyPress={e => e.key === 'Enter' && handleAddItem()}
            placeholder="p.ej. Poción de Vida, Espada Larga..."
            className="flex-1 bg-black/40 border border-gray-600 rounded-lg px-4 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-yellow-500"
          />
          <button
            onClick={handleAddItem}
            disabled={addingItem || !newItem.trim()}
            className="px-4 py-2 bg-yellow-600 hover:bg-yellow-500 disabled:opacity-50 text-gray-900 font-bold rounded-lg transition-colors"
          >
            {addingItem ? '...' : 'Añadir'}
          </button>
          <button
      ...
      {inventory.length === 0 ? (
        ...
        <div className="space-y-3">
          {inventory.map(item => (
            <div
              key={item.id}
              className="bg-gray-800/50 border border-gray-700 rounded-lg p-4 flex items-center justify-between hover:border-yellow-500/50 transition-colors"
            >
              <div className="flex-1">
              ...
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-8 h-8 bg-gray-700 hover:bg-gray-600 rounded text-sm font-bold text-white"
                  >
                    -
                  </button>
                  <span className="w-8 text-center font-bold text-white">{item.quantity || 1}</span>
                  <button
                    onClick={() => handleUpdateQuantity(item.id, 1)}
                    className="w-8 h-8 bg-gray-700 hover:bg-gray-600 rounded text-sm font-bold text-white"
                  >
...
export default function CampaignView() {
  ...
  // Inicializar tab cuando se carga el rol
  useEffect(() => {
    if (userRole && !activeTab) {
      setActiveTab(isGM ? 'notes' : 'mycharacter')
    }
  }, [userRole, isGM])
  ...
  const tabs = isGM
    ...
    : [
      { id: 'mycharacter', label: 'Mi Personaje', icon: <Icon.users /> },
      { id: 'inventory', label: 'Inventario', icon: <Icon.sword /> },
      { id: 'ocr', label: 'OCR', icon: <Icon.ocr /> },
      { id: 'members', label: 'Miembros', icon: <Icon.users /> },
    ]
    ...
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-black overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden px-6 py-5">
        <div className="max-w-6xl mx-auto h-full">
          ...
          {activeTab === 'characters' && <CharactersTab campaignId={campaignId} isGM={isGM} />}
          ...
          {activeTab === 'mycharacter' && <MyCharacterTab campaignId={campaignId} userEmail={user?.email} />}
          {activeTab === 'inventory' && <InventoryTab campaignId={campaignId} userEmail={user?.email} />}
