...
class JoinCampaignRequest(BaseModel):
    """Solicitud para unirse a campaña con código"""
...
async def join_campaign_by_code(
    ...
    try:
        ...
        supabase.client.table("campaign_members").insert(member_data).execute()
        logger.info(f"✅ Usuario {username} ({user_id}) se unió a campaña {campaign['id']} con código {code}")
...
async def get_campaign_members(campaign_id: str, current_user: dict = Depends(get_current_user)):
    """Obtener lista completa de miembros de la campaña"""
    try:
        supabase = get_supabase()
        user_id = current_user["id"]
        
        # Obtener TODOS los miembros de la campaña con datos del usuario
        response = supabase.client.table("campaign_members") \
            .select("user_id, role, joined_at, users(username, email)") \
            .eq("campaign_id", campaign_id) \
            .execute()
        
        # Procesar la respuesta para aplanar estructura
        members = []
        user_role = "PLAYER"  # rol por defecto
        ...
        for item in response.data or []:
            ...
            if user_data:
                ...
                # Si este miembro es el usuario actual, guardar su rol
                if item.get("user_id") == user_id:
                    user_role = item.get("role", "PLAYER")
        
        logger.info(f"✅ Miembros obtenidos para campaña {campaign_id}: {len(members)} miembros")
        ...
        logger.error(f"❌ Error obteniendo miembros de la campaña: {str(e)}")
