...
USING (
    EXISTS (
        SELECT 1 FROM campaign_members
        WHERE campaign_members.campaign_id = session_npcs.campaign_id
        AND campaign_members.user_id = auth.uid()
    )
);
...
-- ============================================================================
-- RLS: session_quests (Phase 2)
-- ============================================================================
CREATE POLICY "Campaign members can view quests"
ON session_quests FOR SELECT
USING (
    EXISTS (
        SELECT 1 FROM campaign_members
        WHERE campaign_members.campaign_id = session_quests.campaign_id
        AND campaign_members.user_id = auth.uid()
    )
);
