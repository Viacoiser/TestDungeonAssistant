    ...
    <div className="h-screen flex flex-col bg-gradient-to-br from-gray-900 via-gray-800 to-black overflow-hidden">
      ...
      <main className="flex-1 overflow-hidden px-6 py-5">
        <div className="max-w-6xl mx-auto h-full">
          ...
          {activeTab === 'voice' && <VoiceTab campaignId={campaignId} />}
...
// ============================================================================
// Tab: Voice — Dictado por Voz
// ============================================================================
export function VoiceTab({ campaignId }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const res = await sessionAPI.listByCampaign(campaignId)
        const activeSessions = res.data.filter(s => s.is_active)
        setSessions(res.data)
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      const { startRecording } = await import('../services/speech.js')
  ...
  const handleStopRecording = async () => {
    try {
      const { stopRecording } = await import('../services/speech.js')
  ...
  const handleSendNote = async () => {
    if (!transcript.trim() || !activeSession) return

    setSending(true)
    setError('')
    setSuccess('')
    try {
      await sessionAPI.addNote(activeSession.id, transcript.trim())
      setTranscript('')
      setInterimTranscript('')
      setSuccess('Nota guardada con éxito')
      setTimeout(() => setSuccess(''), 3000)
    } catch (err) {
      setError(err.message || 'Error al enviar nota')
    } finally {
      setSending(false)
    }
  }
  ...
  return (
    ...
    >
      <div className="max-w-2xl mx-auto flex flex-col gap-8">
        <div className="text-center">
          <h3 className="font-serif text-2xl font-bold text-white mb-2 underline decoration-orange-500/30">Dictado por Voz</h3>
          <p className="text-sm text-gray-400">Graba tus notas usando tu voz - se transcribirán en tiempo real.</p>
        </div>
        ...
        {!activeSession && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-yellow-400 text-sm">
            Necesitas una sesión activa para grabar notas. Comienza una sesión en la pestaña de Notas.
          </div>
        )}
        ...
        <div className="bg-gray-800/40 border border-white/10 rounded-2xl p-8 flex flex-col items-center gap-8">
            ...
            {Array.from({ length: 40 }).map((_, i) => (
              <div
                ...
                className={`w-1 rounded-full transition-all ${
                  recording ? 'bg-red-500' : listening ? 'bg-orange-500' : 'bg-orange-500/30'
                }`}
                style={{
                  height: recording || listening
                    ? `${20 + Math.sin((i + Date.now() / 100) * 0.7) * 14 + Math.cos((i + Date.now() / 100) * 0.4) * 10}px`
                    : `${20 + Math.sin(i * 0.7) * 14 + Math.cos(i * 0.4) * 10}px`
                }}
              />
            ))}
          </div>
          ...
          <button
            ...
            className={`w-20 h-20 rounded-full flex items-center justify-center shadow-lg transition-all duration-300 hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed ${
              recording
          ...
          <p className="text-xs text-gray-500 uppercase tracking-widest">
            {recording ? 'Presiona para detener' : 'Presiona para grabar'}
          </p>
        </div>
        ...
        {(transcript || interimTranscript) && (
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs uppercase tracking-widest text-gray-400 font-bold">Transcripción</span>
            </div>
            ...
            {transcript && (
              <button
                ...
                className="text-xs text-gray-400 hover:text-gray-300 transition self-start"
              >
                Limpiar
              </button>
            )}
        ...
        {transcript && activeSession && (
          <button
            ...
            disabled={sending}
            className="flex items-center justify-center gap-2 py-3 bg-orange-600 hover:bg-orange-500 disabled:bg-orange-600/50 disabled:cursor-not-allowed text-white font-bold rounded-xl transition-all shadow-lg shadow-orange-500/20 uppercase tracking-widest text-xs"
          >
            <Send size={16} />
            {sending ? 'Enviando...' : 'Enviar como Nota'}
          </button>
        )}
      </div>
    </motion.div>
