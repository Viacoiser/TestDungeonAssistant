...
export function NpcsTab({ campaignId, isGM = false }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-0 md:gap-4 h-full relative overflow-hidden">
      ...
      {/* Detalle del NPC */}
      <div className={`
        ${!selectedNpc ? 'hidden md:block' : 'block'}
        flex-1 overflow-y-auto relative h-full
      `}>
        {!selectedNpc ? (
...
// ============================================================================
export function CharactersTab({ campaignId, isGM, user }) {
  ...
  return (
    <div className="flex flex-col md:flex-row gap-0 md:gap-4 h-full overflow-hidden relative">
          ...
          characters.map((char) => (
            ...
            >
              <div style={{
                ...
                <div style={{ flex: 1 }}>
                  <div style={{
                    color: selectedCharacterId === char.id ? '#ffffff' : '#fff',
                    fontWeight: 700,
                    fontSize: '0.95rem',
                    margin: 0,
                    fontFamily: 'Almendra, serif',
      ...
      <div className={`
        ${!selectedCharacterId ? 'hidden md:flex' : 'flex'}
        flex-1 overflow-hidden items-center justify-center relative flex-col
      `}>
        {!selectedCharacter ? (
          ...
          <div className="w-full h-full flex flex-col relative">
            <div className="md:hidden flex-shrink-0 w-full p-2 border-b border-white/10 bg-black/60 sticky top-0 z-10">
              <button 
                ...
                onUpdate={() => {
                  ...
                  const load = async () => {
                    try {
                      ...
                      console.error('Error recargando personajes:', e)
                    }
                  }
                  load()
                }}
...
export default function CampaignView() {
  ...
  return (
    <div className="h-screen flex flex-col lg:flex-row bg-gradient-to-br from-gray-900 via-gray-800 to-black overflow-hidden">
      ...
      {/* Header */}
      <header className="flex-shrink-0 bg-black/60 border-b border-purple-500/20 px-6 py-5">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-6">
          <div className="flex items-center gap-4 flex-1 min-w-0">
            <button
              onClick={() => navigate('/dashboard')}
              className="p-2 rounded-lg text-gray-400 hover:text-white hover:bg-gray-700/50 transition flex-shrink-0"
            >
              <Icon.back />
            </button>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl font-bold text-white truncate">{campaign?.name || 'Campaña'}</h1>
                {userRole && (
                  <span style={{
              ...
              {campaign?.description && (
                <p className="text-sm text-gray-400 mt-1 truncate max-w-xl">{campaign.description}</p>
              )}
            </div>
          </div>

          {/* User Info Section */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', paddingLeft: '1.5rem', borderLeft: '1px solid rgba(255,255,255,0.08)', flexShrink: 0 }}>
      ...
      {/* Tab Content */}
      <main className="flex-1 overflow-hidden px-6 lg:py-5 pt-5 pb-24">
