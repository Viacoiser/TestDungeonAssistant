...
export default function Dashboard() {
  ...
  const handleCreateCampaign = async (e) => {
    ...
    try {
      ...
      const errorMsg = err.response?.data?.detail || err.message || 'Error al crear la campaña'
      ...
      {selectedCampaign ? (
        ...
        <main style={{ flex: 1, padding: '2.5rem 2rem 3rem', overflowY: 'auto', position: 'relative', zIndex: 10 }}>
          <div style={{ maxWidth: 960, margin: '0 auto' }}>
            ...
            {/* ── Contenido por Tab ── */}
            {activeTab !== 'home' ? (
              <SidebarTabContent tab={activeTab} campaigns={campaigns} />
            ) : (
              <>
                {/* Section header */}
...
/* ── Characters Global Tab (all campaigns) ── */
function CharactersGlobalTab({ campaigns }) {
  const navigate = useNavigate()
  ...
  const loadAllCharacters = async () => {
    ...
    try {
      ...
      for (const campaign of campaigns) {
        try {
          const res = await characterAPI.list(campaign.id)
          const chars = res.data?.characters || []
          allChars.push(...chars.map(c => ({ ...c, campaign_id: campaign.id, campaign_name: campaign.name })))
        } catch (e) {
          console.error(`Error cargando personajes de campaña ${campaign.id}:`, e)
        }
      }
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '400px' }}>
        <div style={{ width: 32, height: 32, border: '3px solid rgba(217,83,30,0.3)', borderTop: '3px solid rgba(217,83,30,1)', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <h2 style={{ fontFamily: 'Cinzel, serif', fontSize: '1.8rem', color: '#fff', margin: 0 }}>🧙 Tus Personajes</h2>
        <button
          onClick={() => navigate('/create-character')}
          style={{
            display: 'flex', alignItems: 'center', gap: '0.5rem',
            padding: '0.75rem 1.5rem', borderRadius: 10,
            background: 'rgba(217,83,30,0.8)', color: '#fff',
      ...
      {/* Characters list */}
      {characters.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '3rem 2rem', background: 'rgba(26,26,26,0.4)', borderRadius: 15, border: '1px solid rgba(255,255,255,0.07)' }}>
          <p style={{ fontSize: '3rem', marginBottom: '1rem' }}>🎭</p>
          <p style={{ color: 'rgba(226,209,166,0.6)', marginBottom: '1.5rem' }}>Aún no tienes personajes creados</p>
          <button
            onClick={() => navigate('/create-character')}
            style={{
              padding: '0.75rem 1.5rem', borderRadius: 10,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '1.5rem' }}>
          {characters.map(char => (
            <div
              key={char.id}
              style={{
                background: 'rgba(26,26,26,0.65)', borderRadius: 12,
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow = '0 8px 24px rgba(217,83,30,0.15)'
              }}
              onMouseLeave={e => {
                e.currentTarget.style.borderColor = 'rgba(255,255,255,0.08)'
              ...
              onClick={() => navigate(`/campaign/${char.campaign_id}`)}
            >
              <h3 style={{ fontFamily: 'Cinzel, serif', fontSize: '1.1rem', color: '#fbbf24', margin: '0 0 0.5rem 0' }}>
                {char.name}
              </h3>
              <p style={{ color: 'rgba(226,209,166,0.6)', fontSize: '0.85rem', margin: '0 0 1rem 0' }}>
                📜 Campaña: {char.campaign_name}
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', fontSize: '0.8rem', color: 'rgba(226,209,166,0.5)' }}>
                <div>
                  <span style={{ color: 'rgba(226,209,166,0.4)' }}>Raza:</span> {char.race}
                </div>
                <div>
                  <span style={{ color: 'rgba(226,209,166,0.4)' }}>Clase:</span> {char.class_}
                </div>
                <div>
                  <span style={{ color: 'rgba(226,209,166,0.4)' }}>Nivel:</span> {char.level}
                </div>
                <div>
                  <span style={{ color: 'rgba(226,209,166,0.4)' }}>HP:</span> {char.hp_current}/{char.hp_max}
                </div>
              </div>
            </div>
          ))}
...
function SidebarTabContent({ tab, campaigns }) {
  const navigate = useNavigate()
  const tabLabels = {
  ...
  if (tab === 'characters') return <CharactersGlobalTab campaigns={campaigns} />
