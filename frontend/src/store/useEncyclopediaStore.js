  ...
  persist(
    (set, get) => ({
      ...
      prefetchCategory: async (category) => {
        ...
        try {
          const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/dnd5e/search?q=&categories=${category}&limit=100`)
          if (response.ok) {
            const data = await response.json()
            const results = data.results.map(r => ({ ...r.data, category: r.category })) || []
