...
export default function TraitsReference() {
  ...
  // Categorías disponibles - Combinar tanto categorías básicas como tipos de rasgos
  const categories = ['all', 'races', 'classes', 'backgrounds', 'proficiencies', 'features', 'traits', 'feats']
  ...
  const searchTraits = (query, category) => {
    ...
    if (category !== 'all') {
      // Primero intentar buscar por trait_type (nuevos tipos: races, classes, backgrounds, proficiencies)
      let filtered = results.filter(r => r.trait_type === category)
      // Si no hay resultados, buscar por category (tipos antiguos: features, traits, feats)
