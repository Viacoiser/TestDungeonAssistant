...
export default function EquipmentReference() {
  ...
  // Categorías funcionales unificadas - Incluir nuevos tipos (armor, transport)
  const types = ['all', 'weapon', 'armor', 'transport', 'adventuring-gear', 'tools', 'magic']
  ...
  const searchItems = (query, type) => {
    ...
    if (type !== 'all') {
      // Primero buscar por equipment_type (nuevos: armor, transport)
      let filtered = results.filter(item => item.equipment_type === type)
      
      // Si no hay resultados, buscar por equipment_category.index o category (antiguos)
