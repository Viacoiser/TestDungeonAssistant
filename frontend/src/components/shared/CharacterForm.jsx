...
export default function CharacterForm({ campaignId, onSubmit, loading }) {
  ...
  // Get unique categories from encyclopedia data
  ...
  // Initialize selectedCategory to first available category
  ...
  const handleAddEquipment = (item) => {
    ...
    if (!startingEquipment.find(i => i.id === item.id)) {
      const newItem = {
        id: item.id || Date.now().toString(),
        name: item.name,
        quantity: 1,
        type: item.equipment_type || item.equipment_category?.name,
  ...
  // Filter equipment from encyclopedia data
  const filteredItems = searchTerm 
    ...
    : equipmentData.filter(item => {
        ...
        return itemType === selectedCategory
      })

  // Filter traits from encyclopedia data
  const filteredTraits = traitsData.filter(trait => {
    let matches = true
    if (selectedTraitCategory !== 'all') {
      // trait_type field indicates the category: 'races', 'classes', 'backgrounds', 'proficiencies'
      const traitType = trait.trait_type || trait.category
      matches = traitType === selectedTraitCategory
    }
    if (searchTerm) {
      matches = matches && trait.name.toLowerCase().includes(searchTerm.toLowerCase())
    }
    return matches
  }).sort((a, b) => a.name.localeCompare(b.name))

  // Filter spells from encyclopedia data
  const filteredSpells = spellsData.filter(spell => {
    ...
    if (searchTerm) {
      matches = matches && spell.name.toLowerCase().includes(searchTerm.toLowerCase())
    }
    return matches
  }).sort((a, b) => a.name.localeCompare(b.name))
