...
export default function SpellsReference() {
  ...
  const searchSpells = (query, level) => {
    setLoadingCategory(true)
    let results = spellsData
    
    if (level !== 'all') {
      const levelNum = level === 'cantrip' ? 0 : parseInt(level)
      results = results.filter(s => s.level === levelNum)
    }
  ...
  const handleSelectSpell = (spell) => {
    ...
    // Al usar JSON local, todo el detalle ya viene en el objeto
  ...
  return (
    ...
    <div className="w-full max-w-7xl mx-auto h-full">
      ...
      {/* Lista de Hechizos - Lado Izquierdo */}
          ...
          {filteredSpells.length === 0 ? (
            ...
            filteredSpells.map((spell) => (
              <button
                ...
                style={{
                  background: selectedSpell?.id === spell.id ? 'rgba(217,83,30,0.15)' : 'rgba(255,255,255,0.03)',
                  border: selectedSpell?.id === spell.id ? '2px solid rgba(217,83,30,0.5)' : '1px solid rgba(255,255,255,0.08)',
                  borderRadius: 10,
                  padding: '1rem',
                  transition: 'all 0.2s ease',
                  cursor: 'pointer',
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                ...
                <div style={metaBoxStyle}>
                  ...
                  <div style={{ color: 'var(--fantasy-gold)', fontWeight: 700 }}>{spellDetails.school?.name || spellDetails.school}</div>
                </div>
                <div style={metaBoxStyle}>
