...
class RAGManager:
    ...
    async def register_entity(
        ...
        try:
            ...
            if existing.data and len(existing.data) > 0:
                ...
                result = await self.db.table("rag_entities").update({
    ...
    async def register_relationship(
        ...
        try:
            result = await self.db.table("rag_relationships").upsert({
                "campaign_id": campaign_id,
    ...
    async def create_event_summary(
        ...
        try:
            result = await self.db.table("rag_events").upsert({
                "campaign_id": campaign_id,
    ...
    async def get_campaign_context(
        ...
        try:
            ...
            entities_result = await self.db.table("rag_entities").select("*").match({
                "campaign_id": campaign_id
            ...
            relationships_result = await self.db.table("rag_relationships").select("*").match({
                "campaign_id": campaign_id
            }).order("strength", desc=True).limit(20).execute()
            
            # Obtener eventos (últimas sesiones)
            events_result = await self.db.table("rag_events").select("*").match({
                "campaign_id": campaign_id
    ...
    async def search_entities(
        ...
        try:
            ...
            result = await q.ilike("entity_name", f"%{query}%").execute()
    ...
    async def get_related_entities(
        ...
        try:
            ...
            relationships = await self.db.table("rag_relationships").select("*").match({
                "campaign_id": campaign_id
            ...
            # Get entities
            if related_ids:
                related_entities = await self.db.table("rag_entities").select("*").in_("id", list(related_ids)).execute()
            else:
